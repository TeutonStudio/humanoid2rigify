...
class ErzeugungsModus(str, Enum):
    ORIGINAL_BEHALTEN = "ORIGINAL_BEHALTEN"
    DEFORM_RIG_BEHALTEN = "DEFORM_RIG_BEHALTEN"
    ZU_RIGIFY_UMBAUEN = "ZU_RIGIFY_UMBAUEN"

    @property
    def label(self) -> str:
        if self == ErzeugungsModus.ORIGINAL_BEHALTEN:
            return "Original behalten"

        if self == ErzeugungsModus.DEFORM_RIG_BEHALTEN:
            return "Auf Deform-Rig reduzieren"

        if self == ErzeugungsModus.ZU_RIGIFY_UMBAUEN:
            return "Zu Rigify umbauen"
    ...
    def beschreibung(self) -> str:
        if self == ErzeugungsModus.ORIGINAL_BEHALTEN:
            return (
                "Das ursprüngliche Skelett bleibt erhalten und wird "
                "per Constraints vom neuen Rigify-Rig gesteuert."
            )

        if self == ErzeugungsModus.DEFORM_RIG_BEHALTEN:
            return (
                "Das ursprüngliche Skelett wird auf Deform-Knochen reduziert "
                "und vom neuen Rigify-Rig gesteuert."
            )

        if self == ErzeugungsModus.ZU_RIGIFY_UMBAUEN:
            return (
                "Das ursprüngliche Skelett wird direkt zu einem Rigify-Rig "
                "umgebaut oder damit verschmolzen."
            )
    ...
    def needs_backup(self) -> bool:
        return self in {
            ErzeugungsModus.DEFORM_RIG_BEHALTEN,
            ErzeugungsModus.ZU_RIGIFY_UMBAUEN,
        }

    @classmethod
    def blender_items(cls) -> list[tuple[str, str, str]]:
        return [
            (
                modus.value,
                modus.label,
    ...
    @classmethod
    def from_blender(cls, value: str) -> "ErzeugungsModus | None":
        try:
            return cls(value)
        except ValueError:
            return None
...
PROPS = [
    ...
    (
        ...
        EnumProperty(
            ...
            items=ErzeugungsModus.blender_items(),
            default=ErzeugungsModus.ORIGINAL_BEHALTEN.value,
        ),
    ),
